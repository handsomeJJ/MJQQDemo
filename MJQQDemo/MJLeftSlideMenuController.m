...
#import "MJLeftSlideMenuController.h"
...
NSTimeInterval const kMJSlideMenuDuration = .2f;

@interface MJLeftSlideMenuController ()
...
@implementation MJLeftSlideMenuController
...
- (void)openLeftMenu {
    ...
    MJWeakSelf();
    [UIView animateWithDuration:kMJSlideMenuDuration animations:^{
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)pan {
    ...
    if (pan.state == UIGestureRecognizerStateEnded) {
        ...
        if (mainVCLeft < (kScreenWidth-kMainVCShowWidth) / 2.f) {
            [self closeLeftMenu];
        } else {
            [self openLeftMenu];
        }
